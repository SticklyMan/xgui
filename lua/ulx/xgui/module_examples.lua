...
--Step 1: Create a panel (or Xpanel if you want to use TextEntries!), and set it's parent to xgui.null. It will parent itself to xgui.base later (unless something goes horribly wrong)
local module_tab = x_makeXpanel{ parent=xgui.null }
...
module_tab.listostuff = x_makepanellist{ x=10, y=10, w=200, h=100, parent=module_tab }
module_tab.listostuff:AddItem( x_makeslider{ max=666, label="The slider of DOOM!", decimal=2 } )
module_tab.listostuff:AddItem( x_makecheckbox{ label="Save the whales?" } )
...
module_tab.listostuff = x_makecombobox{ x=10, y=150, w=100, h=200, parent=module_tab }
...
module_tab.XGUI_Refresh = function()
	module_tab.listostuff:Clear()
	for _, v in pairs( player.GetAll() ) do
		module_tab.listostuff:AddItem( v:Nick() )
	end
	module_tab.listostuff:AddItem( "The Slider is at.." )
	module_tab.listostuff:AddItem( module_tab.listostuff:GetItems()[1]:GetValue() )
end
...
table.insert( xgui.modules.tab, { name="Test Module", panel=module_tab, icon="gui/silkicons/group", tooltip=nil, access=nil } )
...
xgui.data.gamemodes	- A list of gamemodes installed on the server
xgui.data.votemaps	- A list of votemaps (also the only maps visible to non-admins via maps menu)
xgui.data.maps		- A list of every map on the server (Must have access to ulx map or access to enable/disable votemaps (ulx_cl_votemapEnabled))
xgui.data.gimps		- A list of gimpsays (Must have access to xgui_svsettings)
xgui.data.adverts	- A list of adverts and data (Must have access to xgui_svsettings)
xgui.data.users		- A list of all users in groups by STEAMID, along with any restrictions (Must have access to xgui_managegroups)
xgui.data.bans		- A list of bans and information (Must have access to xgui_managebans
