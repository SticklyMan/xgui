...
x_makecheckbox{ x=10, y=10, label="Enable Noclip", convar="rep_sbox_noclip", parent=sbox_settings, textcolor=color_black }
x_makecheckbox{ x=10, y=30, label="Enable Godmode", convar="rep_sbox_godmode", parent=sbox_settings, textcolor=color_black }
	...
	for g, limits in ipairs( xgui.data.sboxlimits ) do
		if #limits > 0 then
			...
			for _, cvar in ipairs( limits ) do
				...
				x_makeslider{ x=5+(i%2*210), y=5+math.floor(i/2)*45, w=200, h=45, label="Max " .. cvardata[1]:sub(9), min=0, max=cvardata[2], convar="rep_"..cvardata[1], parent=panel, textcolor=color_white }
				i = i + 1
			end
